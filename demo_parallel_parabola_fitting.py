...
def create_parabola_points(coeffs, t_range=np.arange(0, 20, 0.5), noise_mean_std=[0, 0.5], random_seed=2021):
    assert len(coeffs) == 3
    np.random.seed(random_seed)

    xys = np.zeros((len(t_range), 2), dtype=t_range.dtype)
    xys[:, 0] = t_range
    noise = np.random.normal(*noise_mean_std, t_range.shape)
...
def create_data():
    ...
    xys_list[0] = create_parabola_points(
        (a, b, c0), t_range=np.arange(-15, 15, 0.8), noise_mean_std=[-0.4, 0.5], random_seed=2021
    )

    xys_list[1] = create_parabola_points(
        (a, b, c1), t_range=np.arange(-15, 15, 0.8), noise_mean_std=[-1.0, 1.5], random_seed=2022
    )
    return xys_list
